...
# 16 KB user_data limit; the inline blob alone pushes the rendered script just
# past it, so the whole user_data is gzipped and passed via user_data_base64.
# cloud-init detects the gzip magic bytes and transparently decompresses before
# running the script, so the bootstrap logic below is unchanged.
################################################################################
...
resource "aws_instance" "this" {
  ...
  # Gzipped so the rendered script clears EC2's 16 KB user_data limit; cloud-init
  # decompresses it at boot. base64gzip -> base64(gzip(...)), which is exactly the
  # encoding user_data_base64 expects.
